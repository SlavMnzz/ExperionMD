...
async function handler(m, { conn, args, text , usedPrefix, command }) {
  ...
  try {
    ...
    if (user.health == 0 || user.stamina == 0 || user.mana == 0) return conn.reply(m.chat, `your stamina/healt/mana is less than 100`, m, {
    contextInfo: {
      externalAdReply: {
        title: namebot,
        body: "",
...
handler.before = async m => {
  ...
  let timers = (cooldown - (new Date - user.lastmisi))
  if(new Date - user.lastmisi <= cooldown) return conn.reply(m.chat, `Wait for 🕐${clockString(timers)}`, m, {
    contextInfo: {
      externalAdReply: {
        title: namebot,
        body: "",
